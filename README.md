...
- `search`, `search_plus_interface` и `search_v2` - пока что скрытая функция без интерфейса по поиску во всех текстовых файлах введённого значения (как в pycharm по dabble click Shift);
- `structure_reader` и `obsidian_structure_and_file_reader` - Такая же скрытая функция которая считывает и сохраняет структуру папок и файлов в указанной директории в json файл;
- `TheGamer` - класс для расчёта игрового опыта в реальных навыках (преобразователь затраченных часов в опыт и уровни навыков);
- `WorkFolderCheckSizeManage` - классовый модуль для расчёта размеров всех папок в указанной директории и их вывод списком для дальнейшей работы с ними.
- 